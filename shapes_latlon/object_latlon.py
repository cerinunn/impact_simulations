...
class Object(ABC):
    ...
    def _gen_obj(self):
        ...
        # 
        # print('updated x_loop, y_loop, z_loop',x_loop, y_loop, z_loop)
        # 
        ...
        x_centre, y_centre, z_centre = latlon_to_cartesian_radius(lat=self.loc[2], long=self.loc[1], radius=self.loc[0],
                                                              e2=0, a=self.m.a)
        print('loc', self.loc[0], self.loc[1],self.loc[2] )
        print('x_centre, y_centre, z_centre', x_centre, y_centre, z_centre)


        self._in_shape_condition_simple(x_centre,y_centre,z_centre)
        ...
        #         for j in np.arange(-y_loop, y_loop + 1):
                    ...
                    # cart_coords = self._get_cartesian_coords(i,j,k,x_centre,y_centre,z_centre)
                    ...
                    # shape[int(i) + x_loop, int(j) + y_loop, int(k) + z_loop] = _in_shape_condition_simple(self, i,j,k,x_centre,y_centre,z_centre):
    ...
    def _get_iter_no(self):
        ...
        # for spherical models, the x_loop is radius (in meters)
        x_loop = int(self.radius[0] * self.expand_int  // (self.m.dx))
        y_loop = int(self.radius[1] * self.expand_int  // (metric*self.m.dy))
        z_loop = int(self.radius[2] * self.expand_int  // (metric*self.m.dz))

        print('x_loop, y_loop, z_loop',x_loop, y_loop, z_loop)

        return x_loop, y_loop, z_loop



    def _get_cartesian_coords(self, i, j, k, x_centre, y_centre, z_centre):
        if self.m.type == "SPHERICAL":
            # print('big array')
            # sph_coords = np.array([i*self.m.dx + self.loc[0], j*self.m.dy + self.loc[1], k*self.m.dz + self.loc[2]])
            # print('sph_coords', sph_coords)
            # # print(i, j, k)
            # # print('self.loc[0]', self.loc[0], self.loc[1], self.loc[2])
            # # print('self.m.dx', self.m.dx, self.m.dy, self.m.dz)
            # 
            # # Convert spherical to cartesian:
            # # x, y, z = latlon_to_cartesian(lat=sph_coords[0], long=sph_coords[1], depth=sph_coords[2], e2=0, a=self.m.a)
            # x, y, z = latlon_to_cartesian_radius(lat=sph_coords[2], long=sph_coords[1], radius=sph_coords[0], e2=0, a=self.m.a)
            # 
            # 
            # 
    ...
    def _get_metric(self):
        if self.m.type == "CARTESIAN":
            ...
            # calculate one degree 
            metric = (self.m.a)*np.pi/180
            print('metric', metric)
        else:
